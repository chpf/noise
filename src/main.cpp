...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
	...
	HWND hwnd = CreateWindowEx(
		...
		WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX, // no fullscreen, no resizedwStyle,            // Window style
		//x,           y,             width, height,
	...
	while (running) {
		...
		ScreenToClient(hwnd, &cursor_pos); // can be negative when outside of window
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (uMsg) {
		...
		case WM_PAINT: {
			...
			DrawText(hdc, fromString(std::string("AAAAAAA")), -1 , &rec, DT_TOP|DT_CENTER);
